    ...
    <div v-else>
      ...
      <div class="card">
        ...
        <div class="chart-container">
          <div class="bar-chart">
            <div v-for="item in monthlyData" :key="item.month" class="bar-wrapper">
              <div class="bar-container">
                <div
                  class="bar"
                  :style="{ height: getBarHeight(item.revenue) + 'px' }"
                  :title="currencySymbol + formatNumber(item.revenue)"
                ></div>
              </div>
      ...
      <div class="card">
        ...
        <div class="table-container">
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="(item, index) in monthlyData" :key="item.month">
                <td><strong>{{ formatMonth(item.month) }}</strong></td>
                <td>{{ item.order_count }}</td>
                <td>{{ currencySymbol }}{{ formatNumber(item.revenue) }}</td>
                <td>
                  <span v-if="index > 0" :class="getChangeClass(item.revenue, monthlyData[index - 1].revenue)">
                    {{ getChangeValue(item.revenue, monthlyData[index - 1].revenue) }}
                  </span>
                  <span v-else>-</span>
                </td>
                <td>
                  <span v-if="index > 0" :class="getChangeClass(item.revenue, monthlyData[index - 1].revenue)">
                    {{ getGrowthRate(item.revenue, monthlyData[index - 1].revenue) }}
                  </span>
                  <span v-else>-</span>
                </td>
              </tr>
...
export default {
  ...
  setup() {
    ...
    // Summary stats as computed properties (derived from raw data, no extra state)
    ...
    // Max revenue used for bar height scaling — computed so it updates reactively
    ...
    const formatMonth = (monthStr) => {
      ...
      return key ? `${t('months.' + key)} ${parts[0]}` : monthStr
    }
